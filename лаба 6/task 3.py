...
def to_csv_file(filename, headers = [], rows = [], delimiter=",", new_line="\n"):
    with open(filename, "w") as new:
        new.write(delimiter.join(headers) + new_line)
        new.writelines(delimiter.join(line) + new_line for line in rows)
...
with open(OUTPUT_FILE) as over_writing:
    for line in over_writing:
        print(line, end="")
